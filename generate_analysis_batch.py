...
INDEX_HTML = f'{QUIZ_DIR}/index.html'
ORGANIC_JSON = f'{QUIZ_DIR}/organic_solvent.json'
LAWS_DIR = f'{QUIZ_DIR}/職安法_txt'

def load_or_extract_json():
    """載入獨立JSON，若不存在則從index.html抽出"""
    ...
    save_json(data)
    print(f"已從 index.html 抽出到 {ORGANIC_JSON}")
    return data
...
def save_back_to_html(data):
    ...
    with open(INDEX_HTML, 'w', encoding='utf-8') as f:
        f.write(new_html)
    print(f"已同步回寫 index.html，備份於 {INDEX_HTML}.{suffix}.bak")
...
def main():
    ...
    parser.add_argument('--sync', action='store_true', help='處理完後同步回 index.html')
